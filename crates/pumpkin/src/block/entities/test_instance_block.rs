...
impl TestInstanceBlockBlockEntity {
    ...
    pub async fn error_markers(&self) -> Vec<TestInstanceErrorMarker> {
        self.error_markers.read().await.clone()
    }
    ...
    pub async fn clear_error_markers(&self) {
        let mut markers = self.error_markers.write().await;
        if !markers.is_empty() {
            markers.clear();
            self.dirty.store(true, Ordering::Release);
        }
...
impl BlockEntity for TestInstanceBlockBlockEntity {
    ...
    fn write_nbt<'a>(
        ...
        Box::pin(async move {
            nbt.put_compound("data", encode_data(&self.data().await));
            let markers = self.error_markers().await;
            if !markers.is_empty() {
                nbt.put(
                    "errors",
